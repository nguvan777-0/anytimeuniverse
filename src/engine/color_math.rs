...
//!
//! Stores the 12 founder anchor colors and their original 14D weights.
//! Forks compute their color by taking the delta of their current weights
//! vs their founder's weights, and applying that as an RGB shift from the anchor.
...
pub fn build(
    ...
    // Indices 12..59: weights (4 vecs per founder)
    for i in 0..12 {
        for j in 0..14 {
            flat[12 + i * 4 + j / 4][j % 4] = founder_weights[i][j];
        }
    }
...
pub fn apply(cd: &ColorData, weights: &[f32; 14], founder_id: usize) -> [f32; 3] {
    let id = founder_id % 12;
