...
// 12 founder lineages — each has one dominant weight channel (4..16)
// that locks in its color. weights can mutate later.
pub const FOUNDERS: usize = 12;
...
pub fn init_world(seed: &str) -> (World, f32, [[f32; 14]; 12]) {
    ...
    // spawn 12 founders at random positions, saving their weight vectors
    // so the UI can calibrate the color projection matrix.
    let mut founder_weights = [[0.0f32; 14]; 12];
    for i in 0..FOUNDERS {
        ...
        for c in 4..18 {
            ...
            px[c] = v;
            founder_weights[i][c - 4] = v;
        }

        // Store true descent ID in the new channel 18
        px[18] = i as f32;
    }
    (w, substrate_noise, founder_weights)
}
...
fn main() {
    ...
    if cli_args.iter().any(|a| a == "--help" || a == "-h") {
        println!("anytime-universe v{}", env!("CARGO_PKG_VERSION"));
        println!();
        println!("USAGE:");
        ...
        println!("  ← →                 scrub backward / forward in T");
    ...
    let (w, substrate_noise, founder_weights) = init_world(&seed);
    ...
    println!(
        ...
        FOUNDERS, 1024, 1024, seed, substrate_noise
    );
