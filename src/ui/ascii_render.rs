    ...
    while i < bytes.len() {
        match bytes[i] {
            ...
            27 if bytes.get(i + 1) == Some(&b'[') => {
                match bytes.get(i + 2) {
                    Some(&b'C') => c.scrub_fwd(),   // →  scrub forward
                    Some(&b'D') => c.scrub_back(),  // ←  scrub backward
    ...
    loop {
        ...
        if tape {
            ...
            if (s.t / s.step) as u64 % 32 == 0 {
                let _ = write!(buf,
                    "{:>12}{SEP}{:>8}{SEP}{:>5} {:>4} {:>4} {:>7}{SEP}{:>5} {:>4} {:>4} {:>7}{SEP}{:>5} {:>4} {:>4} {:>7}{SEP}{}\n",
                    "T", "seed",
                    "W0:gn", "bits", "pwr", "energy",
                    "W1:gn", "bits", "pwr", "energy",
                    "W2:gn", "bits", "pwr", "energy",
                    "dominant");
                buf.push_str(&"─".repeat(100));
                buf.push('\n');
            }
            ...
            // dominant: active lineage with highest power
            ...
            let dom_str = match dominant {
                ...
                Some(i) => format!("W{}{}", i, if wave_data[i].1 > 0.95 { " PEAK" } else { "" }),
            };
            ...
            if active_count == 0 {
                ...
                for i in 0..3usize {
                    if wave_data[i].2 <= 0.0 && wave_data[i].0 > 0 {
                        let _ = write!(annotation, " ← W{i} last signal");
                        break;
                    }
                }
            }
            ...
            for i in 0..3usize {
                ...
                let _ = write!(buf, "{SEP}{:>5} {:>4} {:>4.2} {}", gn, bits, pwr, e);
            }
            let _ = write!(buf, "{SEP}{}{}\n", dom_str, annotation);
            ...
            // controls hint
            let _ = write!(buf,
                "\x1b[2K\r\x1b[2m  space:pause  ←→:scrub  ↑↓:speed  1-5:preset  r:rewind  c:seed  q:quit\x1b[0m\n");
        }
