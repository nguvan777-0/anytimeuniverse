...
pub fn run(
    ...
    let color_data = crate::engine::color_math::build(&founder_weights, &espresso_rgb);
    ...
    let mut app = App {
        ...
        title: "anytime-universe".to_string(),
        title_text: "anytime-universe".to_string(),
    };
    let _ = app
        .sim_handle
        .cmd_tx
...
struct App {
    ...
    lineage_colors: Vec<egui::Color32>,
...
fn wave_dominance_at(env_data: &[f32; 24], t: f64, noise: f32) -> [f32; 3] {
    ...
    // Hash: same constants as shader — (wave_i * 97 + gen * 1031 + ch * 7)
    let uhash = |x: u32| -> u32 {
        let mut v = x ^ (x >> 17);
    ...
    for i in 0..3usize {
        ...
        let copy = |g: u32, ch: u32| -> f64 {
            ...
            let parent = gen_param(wi, g.wrapping_sub(1), ch);
...
/// Compute a lineage's display color by applying the 14×3 color projection matrix.
fn blend_lineage_color(weights: &[f32; 14], color_data: &crate::engine::color_math::ColorData, founder_id: usize) -> egui::Color32 {
    let rgb = crate::engine::color_math::apply(color_data, weights, founder_id);
...
impl App {
    fn update_colors(_queue: &wgpu::Queue, _color_buf: Option<()>, _color_data: &crate::engine::color_math::ColorData) {
        // color_data is no longer bound to the shader — PCA/lineage colors live CPU-side only.
    }
    ...
    fn reset_simulation(&mut self, change_seed: bool) {
        ...
        self.lineage_colors = super::espresso_walk::generate(12, &self.seed, super::espresso_walk::Palette::Bright);
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        Theme::Future.provider().apply_theme(&egui_ctx);
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                        ...
                        .show(ctx, |ui| {
                            ...
                            egui::ScrollArea::vertical().show(ui, |ui| {
                                ...
                                if time_resp.changed() {
                                    // decompose scrub position back into epoch + residual
                                ...
                                if self.theme.provider().collapsible_header(ui, "COLOR RIVER", self.show_lineage) {
                ...
                // Always in sync: scrub, jump, reverse — the river instantly shows that epoch.
